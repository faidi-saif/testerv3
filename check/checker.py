...
class VideoStat(FileStat):
    ...
    def extract_errors(self,arg_preview,arg_non_preview):
        bypass              =  arg_preview['params']['options_mode']['bypass']
        log_path            = arg_non_preview['logs'] + '/rtos.txt'
        shooting_mode       = arg_non_preview['params']['shooting_mode']
        if bypass == 'disable' and shooting_mode == 'spherical':
            rtos_data = self.logger.read(log_path)
            skipped_frames = self.parser.search_pattern(rtos_data,'skipped_frames')
            errors         = self.parser.search_pattern(rtos_data, 'errors')
            return skipped_frames,errors
        else :
            return None
    ...
    def check(self, *args):
        ...
        if files != []:
            ...
            result_flag = False
        scenario_result = {'result': result_flag, 'number of files': files_number, 'files': files_properties}
        errors = self.extract_errors(preview_state,non_preview_state)
        if errors :
            scenario_result.update({'skippped frames':int(errors[0]),'errors':int(errors[1])})
